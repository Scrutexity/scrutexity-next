...
const included = [
  ...
  'BAA before patient-adjacent activation',
];
      ...
      <section className="relative overflow-hidden px-5 pb-16 pt-28 sm:px-8 lg:pb-24 lg:pt-36">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <h1 className="font-display text-4xl leading-[1.05] tracking-[-0.02em] text-espresso sm:text-5xl md:text-[3.8rem]">
            ...
            <span className="text-clay">Without discounting your brand.</span>
          </h1>
          ...
          <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <a href="/pilot" className="clay-cta inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold">Get Your Free Audit</a>
            <Link href="/pricing" className="text-sm font-medium text-mist underline-offset-4 hover:text-espresso hover:underline">
              See pricing
            </Link>
          </div>
        </div>
      ...
      <main className="px-5 pb-24 sm:px-8">
        ...
        <section className="mx-auto max-w-4xl py-16">
          <div className="grid gap-6 md:grid-cols-2">
            <div className="luxury-panel p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-clay-deep mb-4">The Tuesday 11am problem</p>
              <p className="text-[15px] leading-8 text-mist">
            ...
            <div className="luxury-panel p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-clay-deep mb-4">You can't put this on Instagram</p>
              <p className="text-[15px] leading-8 text-mist">
        ...
        <section className="mx-auto max-w-5xl py-8">
          <div className="text-center mb-14">
            ...
            <p className="mt-3 text-mist max-w-xl mx-auto">Three steps, fully automated, logged with source and outcome every time.</p>
          </div>
            ...
            {steps.map((step, i) => {
              ...
              return (
                <div key={step.label} className="luxury-panel p-8 relative">
                  ...
                  <h3 className="font-display text-xl text-espresso mb-3">{step.label}</h3>
                  <p className="text-[15px] leading-7 text-mist">{step.body}</p>
                </div>
              );
            })}
          </div>
        ...
        <section className="mx-auto max-w-3xl py-16">
          <div className="rounded-[1.75rem] border border-[#d8c4ad] bg-cream p-8 md:p-10">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-clay-deep mb-4">What the patient sees</p>
            <div className="rounded-2xl bg-[#f3eadf] p-6 font-sans text-sm leading-7 text-[#3d3731]">
              <p className="text-clay-deep text-xs font-bold uppercase tracking-wider mb-3">SMS · From: Clinic Name</p>
        ...
        <section className="mx-auto max-w-3xl py-8">
          <div className="rounded-[1.75rem] border-2 border-clay/30 bg-cream p-8 md:p-10">
            <p className="section-kicker mb-3">Yield module — Portfolio plan ($3,900/mo)</p>
            <h2 className="font-display text-2xl text-espresso mb-6">Everything in Recover, plus the Flash Fill engine.</h2>
            <ul className="grid gap-3 sm:grid-cols-2">
              {included.map((item) => (
                <li key={item} className="flex items-start gap-2.5 text-sm text-mist">
                  <Check className="h-4 w-4 shrink-0 mt-0.5 text-clay" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="/pilot" className="clay-cta inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold">Get Your Free Audit</a>
              <Link href="/pricing" className="btn-ghost btn-md">
                Compare all plans <ArrowRight className="btn-arrow h-4 w-4" />
              </Link>
            </div>
