      ...
      <section className="relative overflow-hidden px-5 pb-16 pt-28 sm:px-8 lg:pb-20 lg:pt-32">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <h1 className="font-display text-4xl leading-tight tracking-[-0.02em] text-[#201d19] sm:text-5xl md:text-6xl">
            Knowledge engineered for AI retrieval.
          </h1>
          ...
          {articles.map((article) => {
            ...
            return (
              <Link
                ...
                className="group flex items-start gap-5 rounded-2xl border border-[#e1d4c5] bg-[#fffaf2]/80 p-7 shadow-[0_2px_16px_rgba(85,62,41,0.06)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#d8b17a]/60 hover:shadow-[0_8px_32px_rgba(85,62,41,0.10)]"
              >
                <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#e1d4c5] bg-[#f3eadf] text-terracotta">
                  <Icon size={18} />
                </div>
                <div className="flex-1">
                  <span className="section-kicker mb-2 block">{article.category}</span>
                  <h2 className="text-lg font-semibold leading-snug text-[#221f1b] transition-colors group-hover:text-terracotta">
                    {article.title}
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-[#6b6259]">{article.description}</p>
                </div>
                <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-[#c4b4a0] transition-transform group-hover:translate-x-1 group-hover:text-terracotta" />
              </Link>
            );
          })}
        </div>
