...
const STATUS_META: Record<Status, { label: string; dot: string; text: string }> = {
  ...
  beta: { label: 'Beta', dot: 'bg-terracotta', text: 'text-terracotta' },
  roadmap: { label: 'Roadmap', dot: 'bg-charcoal/30', text: 'text-charcoal/55' },
};
...
export default function IntegrationStatus({ heading = true }: { heading?: boolean }) {
  return (
    <section id="platform" className="bg-ivory-2 px-7 py-28">
      <div className="mx-auto max-w-6xl">
        {heading && (
          <div className="mb-12 max-w-[52ch]">
            ...
            <p className="mt-4 text-[1.05rem] leading-relaxed text-charcoal/70">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          {INTEGRATIONS.map((it) => {
            ...
            return (
              <div
                key={it.name}
                className="rounded-2xl border border-line bg-porcelain p-7 shadow-card"
              >
                <div className="flex items-center justify-between">
                  <span className="font-serif text-[1.35rem] font-medium text-charcoal">
                    {it.name}
                  </span>
                ...
                <p className="mt-3 text-[0.92rem] leading-relaxed text-charcoal/65">{it.note}</p>
              </div>
            );
          })}
        </div>
        <p className="mt-6 max-w-[60ch] font-mono text-[0.74rem] leading-relaxed text-charcoal/55">
          A platform is marked Live only when a read-only connection is running in production. Need a
          stack we don&apos;t list yet? <a href="/company#hiring" className="text-terracotta underline underline-offset-2">Talk to us.</a>
        </p>
      </div>
    </section>
  );
