    ...
    <section className="py-24 bg-ivory">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-5xl font-serif tracking-tight text-charcoal mb-4">Your 14-Day Revenue Recovery Sprint</h2>
          <p className="text-xl text-charcoal/70">What actually happens after you request the audit</p>
        </div>

        <div className="grid md:grid-cols-4 gap-8">
          {steps.map((step, i) => (
            <div key={i} className="relative bg-white rounded-3xl p-10 border border-white/60 group hover:border-terracotta/30 transition-all">
              <div className="font-mono text-terracotta font-bold text-2xl mb-4 opacity-60 group-hover:opacity-100 transition-all">{step.icon}</div>
              <div className="font-mono text-terracotta text-sm tracking-widest mb-3">{step.day}</div>
              <h3 className="text-2xl font-semibold text-charcoal mb-4">{step.title}</h3>
