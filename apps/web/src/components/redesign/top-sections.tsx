...
export function MakroNav({
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${
      isLight
        ? 'border-b border-black/5 bg-paper-light/80 backdrop-blur-md text-ink'
        : 'border-b border-white/10 bg-ink/80 backdrop-blur-md text-ink'
    }`}>
      <div className="mx-auto flex max-w-[1200px] items-center justify-between px-5 py-3 sm:px-8">
        <a href="#top" className="flex items-center gap-2">
          ...
          <span className={`text-[15px] font-semibold tracking-tight ${isLight ? 'text-ink' : 'text-ink'}`}>
            Scrutexity
          </span>
        </a>
        ...
        <div className="hidden items-center gap-6 md:flex">
            ...
            {NAV_LINKS.map((l) => (
              <a
                ...
                className={`text-[13px] font-medium transition-colors ${
                  isLight ? 'text-muted hover:text-ink' : 'text-muted hover:text-ink'
                }`}
              >
                {l.name}
              </a>
          ...
          {setHeroOption && (
            <div className={`flex items-center gap-2 rounded-full border px-3 py-1 text-[12px] font-medium ${
              isLight ? 'border-black/10 bg-white/80 text-muted' : 'border-white/15 bg-white/10 text-muted'
            }`}>
              <span>Hero options:</span>
              <div className="flex items-center gap-1 rounded-full bg-ink-soft p-0.5 text-white">
        ...
        <button
          ...
          className={`flex h-9 w-9 items-center justify-center rounded-lg border md:hidden ${
            isLight ? 'border-black/10 text-ink' : 'border-white/10 text-ink'
          }`}
        >
    ...
    <section id="top" className="relative overflow-hidden px-5 pb-20 pt-36 sm:px-8 sm:pt-44">
      ...
      <div
        ...
        className={`pointer-events-none absolute -top-40 left-1/2 h-[560px] w-[900px] -translate-x-1/2 rounded-full blur-[140px] ${
          isLight ? 'bg-blue-100/60' : 'bg-ink-soft/40'
        }`}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute right-[-160px] top-40 h-[360px] w-[360px] rounded-full bg-accent/10 blur-[120px]"
      />
      ...
      <div className="relative mx-auto flex max-w-[1200px] flex-col items-center text-center">
        <Reveal>
          <span className={`inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.22em] shadow-xs ${
            isLight ? 'border-black/10 bg-white text-muted' : 'border-white/10 bg-white/[0.06] text-muted'
          }`}>
        ...
        <Reveal delay={0.08}>
          <h1 className={`mt-6 max-w-[820px] text-[40px] font-bold leading-[1.05] tracking-[-0.03em] sm:text-6xl lg:text-7xl ${
            ...
            Know your claims.
            <br />
            <span className={isLight ? 'text-muted' : 'text-muted'}>Audit with AI.</span>
          </h1>
        </Reveal>
        ...
        <Reveal delay={0.32} className="mt-16 w-full">
          <div className="relative mx-auto max-w-[980px]">
            <div
              aria-hidden
              className="pointer-events-none absolute -inset-6 rounded-[36px] bg-ink-soft/40 blur-[70px]"
            />
            <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-ink/80 shadow-[0_40px_120px_-30px_rgba(0,0,0,0.8)]">
              {/* window chrome */}
              <div className="flex items-center gap-2 border-b border-white/[0.07] px-5 py-3.5">
                <span className="h-2.5 w-2.5 rounded-full bg-paper-light/70" />
                <span className="h-2.5 w-2.5 rounded-full bg-accent/70" />
                <span className="h-2.5 w-2.5 rounded-full bg-paper-light/70" />
                <span className="ml-4 hidden rounded-md border border-white/10 bg-white/[0.05] px-3 py-1 text-[11px] text-muted sm:block">
              ...
              <div className="grid grid-cols-1 md:grid-cols-[180px_1fr]">
                ...
                <aside className="hidden flex-col gap-1 border-r border-white/[0.07] p-4 md:flex">
                  ...
                  <div className="mt-auto rounded-lg border border-white/[0.07] bg-white/[0.04] px-3 py-2.5">
                    ...
                    <p className="mt-1 text-xl font-bold text-accent-text">61</p>
                  </div>
                </aside>
                ...
                <div className="p-5 sm:p-6">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-accent/15 px-3 py-1 text-[10.5px] font-bold uppercase tracking-wide text-accent-text">
                      Live
                    </span>
                  </div>
                  ...
                  <div className="mt-5 space-y-2.5">
                    {[
                      { claim: '“FDA-cleared safety”', status: 'VERIFIED', cls: 'text-accent-text' },
                      { claim: '“Clinically proven results”', status: 'GAP', cls: 'text-muted' },
                      { claim: '“Doctor-recommended”', status: 'UNSUPPORTED', cls: 'text-muted' },
                      { claim: '“Scientifically validated”', status: 'VERIFIED', cls: 'text-accent-text' },
                    ].map((r) => (
                      <div
                        key={r.claim}
                        className="flex items-center justify-between rounded-xl border border-white/[0.07] bg-white/[0.04] px-4 py-3"
                      >
                        <span className="text-[13px] text-ink" dangerouslySetInnerHTML={{ __html: r.claim }} />
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-4 border-t border-white/[0.07] pt-4">
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-muted">Drift signal</p>
                      <p className="text-lg font-bold text-accent-text">+15%</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-muted">Claims reviewed</p>
                      <p className="text-lg font-bold text-ink">64</p>
                    </div>
                    <div className="ml-auto hidden items-center gap-2 sm:flex">
                      <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-accent" />
                      <span className="text-[11px] text-muted">Auto-checking new sources</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <Reveal delay={0.06}>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
            {TRUSTEES.map((t) => (
              <span
                key={t}
                className="text-sm font-semibold uppercase tracking-[0.14em] text-muted/60"
              >
                {t}
              </span>
            ))}
        ...
        <Reveal delay={0.08}>
          <div className="mt-4 grid items-center gap-8 lg:grid-cols-[1.4fr_1fr]">
            <div>
              ...
              <a
                href="/sample-report"
                className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent-text transition-colors hover:text-accent-text"
              >
                Read the case study
                <ArrowRight size={15} />
              </a>
            ...
            <GlassCard className="p-5">
              ...
              <div className="mt-4 space-y-3">
                {[
                  { claim: '&ldquo;FDA-cleared safety&rdquo;', status: 'VERIFIED', tone: 'text-accent-text' },
                  { claim: '&ldquo;Clinically proven results&rdquo;', status: 'GAP', tone: 'text-muted' },
                  { claim: '&ldquo;Doctor-recommended&rdquo;', status: 'UNSUPPORTED', tone: 'text-muted' },
                ].map((r) => (
                  <div
                    key={r.claim}
                    className="flex items-center justify-between rounded-xl border border-white/10 bg-ink/50 px-4 py-3"
                  >
                    <span
