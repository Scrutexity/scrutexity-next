          ...
          {CLARITY_CARDS.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.08} className="h-full">
              <GlassCard isLight={isLight} className="flex h-full flex-col p-7">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-accent text-ink font-bold shadow-xs">
                  <c.icon size={20} />
                </span>
                <h3 className={`mt-5 text-lg font-semibold ${isLight ? 'text-ink' : 'text-ink'}`}>{c.title}</h3>
                <p className={`mt-2 flex-1 text-[13.5px] leading-relaxed ${isLight ? 'text-muted' : 'text-muted'}`}>
                  {c.desc}
                </p>
                <a
                  href="/snapshot"
                  className="mt-5 inline-flex items-center gap-1.5 text-[13px] font-bold text-ink underline transition-colors hover:text-accent-text"
                >
                  Get started
                  <ArrowRight size={14} />
                </a>
