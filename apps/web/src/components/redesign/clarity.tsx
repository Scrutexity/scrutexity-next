          ...
          {CLARITY_CARDS.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.08} className="h-full">
              <GlassCard className="flex h-full flex-col p-7">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#d9ff5c]/15 text-[#d9ff5c]">
                  <c.icon size={20} />
                </span>
                <h3 className="mt-5 text-lg font-semibold text-[#ebedfa]">{c.title}</h3>
                <p className="mt-2 flex-1 text-[13.5px] leading-relaxed text-[#9391b8]">
                  {c.desc}
                </p>
                <a
                  href="/snapshot"
                  className="mt-5 inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#d9ff5c] transition-colors hover:text-[#e4ff85]"
                >
                  Get started
                  <ArrowRight size={14} />
                </a>
